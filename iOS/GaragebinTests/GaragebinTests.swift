import XCTest
@testable import Garagebin

@MainActor
final class GaragebinTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(BinEntry(title: "Test", detail1: "A", detail2: "B", note: "", date: Date()))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(BinEntry(title: "ToDelete", detail1: "A", detail2: "B", note: "", date: Date()))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        while store.entries.count < Store.freeLimit {
            store.add(BinEntry(title: "Filler", detail1: "A", detail2: "B", note: "", date: Date()))
        }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(BinEntry(title: "Original", detail1: "A", detail2: "B", note: "", date: Date()))
