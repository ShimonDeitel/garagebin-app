...
struct ContentView: View {
    ...
    @State private var editingEntry: BinEntry?
            ...
            ZStack {
                ...
                if store.entries.isEmpty {
                    VStack(spacing: 12) {
                        ...
                        Text("Tap + to log your first bin.")
                            .font(Theme.font(15))
...
struct EntryFormView: View {
    ...
    let existingID: UUID?
    let onSave: (BinEntry) -> Void
    @FocusState private var focusedField: Field?

    enum Field { case title, detail1, detail2, note }

    init(entry: BinEntry?, onSave: @escaping (BinEntry) -> Void) {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(existingID == nil ? "Add Bin" : "Edit Bin")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = BinEntry(title: title, detail1: detail1, detail2: detail2, note: note, date: date)
